// commands/server/server_about.js
...
const data = new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand => 
        subcommand
            .setName('server')
            .setDescription('View server description')
    )
    .addSubcommand(subcommand =>
        subcommand
            .setName('edit')
    ...
    .addSubcommand(subcommand => 
        subcommand
            .setName('clear')
            .setDescription('Delete server description and all media')
    );
        ...
        switch (subcommand) {
            case 'server': {
                const result = await db.get(
                    'SELECT * FROM server_about WHERE guild_id = ?', 
                    [guild.id]
                );
            ...
            case 'set': {
                ...
                const existing = await db.get(
                    'SELECT 1 FROM server_about WHERE guild_id = ?', 
                    [guild.id]
                );
                ...
                const fields = [
                    { name: 'Description', value: desc.length > 100 ? `${desc.substring(0, 100)}...` : desc }
                ];
            ...
            case 'edit': {
                ...
                // Process thumbnail: "clear" removes, empty keeps current
                ...
                // Process image: "clear" removes, empty keeps current
                ...
                // Process color: "clear" removes to default, empty keeps current
                ...
                const fields = [
                    { name: 'Description', value: newDesc.length > 100 ? `${newDesc.substring(0, 100)}...` : newDesc }
                ];
